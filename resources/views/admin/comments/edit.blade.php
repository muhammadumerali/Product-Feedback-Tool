...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.comments.update", [$comment->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="feedback_id">{{ trans('cruds.comment.fields.feedback') }}</label>
                <select class="form-control select2 {{ $errors->has('feedback') ? 'is-invalid' : '' }}" name="feedback_id" id="feedback_id" required>
                    @foreach($feedback as $id => $entry)
                        <option value="{{ $id }}" {{ (old('feedback_id') ? old('feedback_id') : $comment->feedback->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.comment.fields.feedback_helper') }}</span>
            </div>
            <div class="form-group">
                <label class="required" for="date">{{ trans('cruds.comment.fields.date') }}</label>
                <input class="form-control date {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text" name="date" id="date" value="{{ old('date', $comment->date) }}" required>
                @if($errors->has('date'))
                    <div class="invalid-feedback">
                        {{ $errors->first('date') }}
                    </div>
